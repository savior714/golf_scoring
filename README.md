...
- **동기화 안정성 강화**: 로그인 직후 다중 비동기 호출 시 발생하는 Race Condition을 해결하기 위해 **Singleton Promise 패턴**을 도입하고, `onAuthStateChange` 세션 객체 직접 전달 방식으로 데이터 정합성 확보.
- **SSR 빌드 호환성 (Vercel)**: Web 정적 렌더링 시 `window` 객체 부재로 인한 빌드 에러를 방지하도록 Supabase 및 스토리지 로직에 **Isomorphic Safety Wrapper** 적용.
