...
- **쓰리퍼트 자동 부여**: 퍼트 3개 이상 시 자동 태깅 / FIFO 관리
...
- **Safe Sync Protocol**: 타임스탬프 + 데이터 완전성 비교로 유실 방지
- **Offline Sync Queue**: 실패 시 자동 재시도 큐잉 (최신순)
...
- **Admin-Only Access**: `isAdmin` 상태 및 RLS 기반 탭 안정성 확보
...
src/modules/golf/
  ...
  golf.repository.ts  # 저장소 (Repository - I/O)
  golf.service.ts     # 로직 (Service - Logic)
  service/            # validateClubData 등 특화 서비스
...
- [navigation_stability.md](./docs/plans/navigation_stability.md): 내비게이션 안정화 목표
