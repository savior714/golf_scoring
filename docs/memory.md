...
- **[2026-03-17: 보안 취약점 감사 전체 완료 및 취약점 보완 적용]**
    - **Task 1~4 (조사)**: RLS 활성, 민감 정보 노출 없음, 클라이언트 변조 방어, XSS 안전성 확인 완료.
    - **Task 5 (부재 확인)**: 하루 10건 생성 제한, 과거 날짜 차단 등 핵심 제약 조건이 DB 수준에서 누락된 점 발견.
    - **Task 6 (보완/Hardening)**: `20260317000000_security_hardening.sql` 작성을 통해 다음 사항을 DB 수준에서 강제함.
        1. **권한 SSOT**: `is_admin()` 함수가 `profiles.role`을 직접 참조하도록 표준화.
        2. **라운드 제약**: 하루 최대 10건 생성 제한 및 과거 날짜 기록 생성 차단 트리거 적용.
        3. **데이터 무결성**: `golf_courses.hole_count = 9` 제약 조건 추가.
    - **파일**: `docs/plans/security_vulnerability_audit.md`, `supabase/migrations/20260317000000_security_hardening.sql`.
