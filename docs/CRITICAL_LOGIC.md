...
- **개인 핸디캡 추정 정책 (Estimated Handicap)**: 사용자의 실력을 정량적으로 파악하기 위해 **USGA 방식의 간이 핸디캡**을 계산한다.
  - **데이터 기준**: 최근 20경기 중 성적이 좋은 상위 25%(최대 5경기)를 추출한다.
...
- **절대 경로 임포트 정책 (Absolute Alias Import Policy)**: Vercel(Linux) 등 특정 빌드 환경에서 괄호가 포함된 디렉토리(`(tabs)`) 경로 해석 오류를 방지하기 위해, 모든 내부 모듈 임포트는 상대 경로(`../`) 대신 프로젝트 루트 기준 **절대 경로 별칭(`@/src/`)**을 사용해야 한다. 이를 강제하기 위해 `babel-plugin-module-resolver`를 도입하며, 세부 규칙은 `docs/SSOT_PATH_CONVENTION.md`를 따른다.
- **아이콘 라이브러리 최적화 (Lucide Direct Import)**: 웹 번들 크기 최소화 및 빌드 성능 향상을 위해 Lucide 아이콘은 반드시 개별 파일 임포트(`lucide-react-native/dist/icons/*`) 방식을 사용한다. v0.576.0 이상의 버전에서는 명명 규칙 및 경로 변경에 유의하며, Metro 설정(`metro.config.js`)을 통해 경로 리다이렉션을 지원한다. 존재하지 않는 아이콘(예: `edit-3`)은 최신 규격(`pen-line`)으로 대체한다.
