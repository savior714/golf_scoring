...
* **마이크로태스크 원칙 (Micro-Task Constraint):**
  ...
  * **복합 작업 금지:** 읽기 + 수정 + 검증을 하나의 Task로 묶지 않는다. 반드시 별개의 Task로 분리한다. (예: `Read` → `Edit` → `Bash` 순으로 각각 별도 태스크 할당)
* **단계별 실행 제약 (Step-Lock Protocol):**
  * **1 Task = 1 응답:** 한 번의 응답에서 **단 하나의 Task만 실행**하고 결과를 보고한다.
  * **실행 후 반드시 멈춰라:** Task 완료 후 체크박스(`[x]`)를 업데이트하고, **다음 Task로 자동 진행을 엄격히 금지**한다.
  * **명시적 승인 대기:** 반드시 사용자의 "다음 진행해줘" 또는 "Task N 진행해줘" 지시를 기다린다.
  * **그룹 단위 실행 금지:** 사용자가 "A 그룹 진행해줘"라고 요청하더라도 그룹 내 첫 번째 Task만 실행하고 멈춘다.
* **모듈화 및 파일 분리 (Modularization):**
  * **분리 기준:** 파일이 **300~500라인 이상**이거나 이름에 `And`, `Manager` 등이 포함되어 책임이 과중할 때 분리를 검토한다.
...
3. **Edit:** .NET 기반 정밀 I/O 수정 (1 Task = 1 Tool Call 준수) 및 ruff/린트 체크
