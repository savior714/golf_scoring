...
### A-5. 홀 메모(Hole Memo) 입력 UI 개선(계획 폐기)
...
### A-6. 라운드 메모(Round Memo) 입력 기능(계획 폐기)
...
### B-1. 전반·후반 각 9홀 합계 소계(Sub-total) 표시(계획 폐기)
...
### B-2. FIR(Fairway In Regulation) 기록 UI 복구(계획 폐기)
...
### B-6. 라운드 복사(Round Duplication) 기능(계획 폐기)
...
### D-5. iCloud / Google Drive 백업 (선택적) -> google drive만으로
...
### G-2. AI 기반 약점 분석 및 연습 추천 (추후 강하게 고려)
