...
export const RecordFooter: React.FC<RecordFooterProps> = memo(({
  currentHole,
  insetsBottom,
  isMounted,
  saveCurrentHole,
  ...
  const handlePrevHole = useCallback(async () => {
    await saveCurrentHole(); 
    if (isMounted) setCurrentHole((h: number) => h - 1); 
  }, [saveCurrentHole, isMounted, setCurrentHole]);
  ...
  }, [saveCurrentHole, isMounted, finishRound]);
  ...
  return (
    <View style={[styles.footer, { paddingBottom: Math.max(insetsBottom, 8) }]}>
      ...
      {currentHole < 18 && (
        <TouchableOpacity style={styles.earlyFinishBtn} onPress={handleEarlyFinish}>
