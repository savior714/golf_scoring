...
export class MatchRepositoryImpl implements MatchRepository {
  ...
  async repairRoundCourseIds(
    ...
    try {
      ...
      if (error || !data) {
        logger.warn(`[MatchRepositoryImpl] repairRoundCourseIds: club not found for "${clubName}"`);
        return EMPTY;
      }
      ...
      for (const pass of [1, 2, 3]) {
        for (const out of courses) {
          for (const inn of courses) {
            ...
            if (matched) {
              logger.info(
                `[MatchRepositoryImpl] repairRoundCourseIds: pass-${pass} match — out:"${out.name}" in:"${inn.name}" (club:"${clubName}")`
              );
      ...
      logger.warn(
        `[MatchRepositoryImpl] repairRoundCourseIds: no match — courseType:"${courseType}" club:"${clubName}"`
      );
      return EMPTY;
    } catch (e: unknown) {
      logger.error('[MatchRepositoryImpl] repairRoundCourseIds unexpected error', e);
      throw new GolfError('STORAGE_ERROR', 'Failed to repair course IDs', e);
    }
  }
}
